...
from .prompt import PromptSpec, build_prompt_spec


def _common_model_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--model", type=Path, default=default_model_path())
    parser.add_argument("--backend", choices=["auto", "generic", "qwen35"], default="auto")
...
def _load_backend(args: argparse.Namespace):
    ...
    return load_backend(
        ...
        device=args.device,
        dtype=args.dtype,
        safe_kernels=not args.no_safe_kernels,
        backend=args.backend,
    )


def cmd_index(args: argparse.Namespace) -> None:
    backend = _load_backend(args) if args.native_dense else None
    index = DocumentIndex.build(
        args.docs,
        generator=backend,
        native_dense=args.native_dense,
...
def cmd_ask(args: argparse.Namespace) -> None:
    ...
    hits = index.search(
        args.question, top_k=args.top_k, generator=backend, rrf_k=args.rrf_k,
        neighbor_radius=args.neighbor_radius, max_context_chunks=args.max_context_chunks,
        candidate_k=args.candidate_k,
        span_size=args.span_size,
    )
    prompt = build_prompt_spec(args.question, hits)
...
def cmd_smoke(args: argparse.Namespace) -> None:
    backend = _load_backend(args)
    result = NativeRAGEngine(backend, args.prefill_slice).generate(
        PromptSpec("Ты кратко отвечаешь на проверочный запрос.", (), "Ответь одним словом: готов."),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    index.add_argument(
        "--native-dense",
        action="store_true",
        help="use native dense features from the same generator while building the index",
    )
